...
| `$chatType` | The type of the chat in which the message was sent. | One of `general`, `whisper`, `say`, `shout`, `faction`, `safehouse`, `radio`, `admin`, or `server`.<br><br>`whisper` refers to private chats, not [local whispers](../sandbox-options/chat-formats.md#chatformatwhisper). |
