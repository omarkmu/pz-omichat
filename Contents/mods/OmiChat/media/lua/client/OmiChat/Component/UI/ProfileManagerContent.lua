---Content panel for the profile manager.
...
function ContentPanel:createCalloutControls(manager, startY)
    ...
    for i = 1, #categories do
        ...
        control:setValidateFunction(control, self.validateCustomCalloutText)
...
function ContentPanel:createColorControls(manager, startY)
    ...
    for i = 1, #availableColorOpts do
        ...
        if not labelText then
            labelText = getText('UI_OmiChat_ContextColor', API.streams.getDisplayCommand(opt))
        end
        ...
        if not tooltip then
            local optName = getTextOrNull('UI_OmiChat_ContextMessageType_' .. opt)
                or API.streams.getDisplayCommand(opt)
...
---@param target omi.ui.TextEntry
---@param text string
---@return boolean
function ContentPanel.validateCustomCalloutText(target, text)
...
function ContentPanel:new(x, y, width, height)
    local this = ISPanelJoypad.new(self, x, y, width, height) ---@cast this omichat.ProfileManagerContent
    return this
end
