...
function Transformation.run(info)
    local player = getSpecificPlayer(0)
    if not player then
        return
    end

    Transformation._setup(info, player)

    if API.hooks.has.beforeDecodeMessage then
        API.hooks.beforeDecodeMessage(info)
    end

    local handled = API.hooks.has.decodeMessage and API.hooks.decodeMessage(info)
    if not handled and not info.hidden then
        Transformation._decodeMentions(info)
        Transformation._decodeCommand(info)
        Transformation._decodeEcho(info, player)
        Transformation._decodeCallouts(info)
        Transformation._decodeStream(info)
        Transformation._decodeLanguages(info, player)
        Transformation._decodeNarrativeStyle(info)
    end

    if API.hooks.has.afterDecodeMessage then
        API.hooks.afterDecodeMessage(info)
    end

    Transformation._formatMentions(info)
    Transformation._checkRange(info, player)
    Transformation._checkTransmitRadio(info)

    info:onTransform()
end


---Handles ranged chat behavior.
---@param info MessageInfo
---@param player IsoPlayer
---@private
function Transformation._checkRange(info, player)
    if info.chatType ~= 'say' and info.chatType ~= 'shout' then
        return
    end

    local stream = info.stream --[[@as ChatStream]]
...
---@param info MessageInfo
---@private
function Transformation._checkTransmitRadio(info)
...
---Decodes information about a callout chat message.
---@param info MessageInfo
---@private
function Transformation._decodeCallouts(info)
...
---Decodes encoded command information.
---@param info MessageInfo
---@private
function Transformation._decodeCommand(info)
    ...
    local match = formatter and formatter:read(info.content or info.rawText)
    if not match then
        return
    end

    if API.hooks.has.decodeCommand and API.hooks.decodeCommand(match, info) then
        return
    end

    local start = match:find(COMMAND_ARGS_START)
    if not start then
        return
    end

    match = match:sub(start + 1)

    local targetStream
    local name = stream.name
    if name == 'card' then
        local suit = utils.decodeInvisibleCharacter(match)
        local card = utils.decodeInvisibleCharacter(match, 2)
        if suit < 1 or suit > 4 or card < 1 or card > 13 then
            info:hide()
            return
        end

        info.content = match:sub(3)
    ...
    elseif name == 'flip' then
        local result = utils.decodeInvisibleCharacter(match)
        if result ~= 1 and result ~= 2 then
            info:hide()
            return
        end

        info.content = match:sub(2)
        info.tokens.heads = result == 1

        targetStream = API.streams.firstChatStreamWithTag('FlipCommandTarget')
    elseif name == 'roll' then
        local seq = utils.decodeInvisibleIntSequence(match, 2)
    ...
    if not targetStream or info.chatType == 'radio' then
        info:hide()
        return
    end
...
---Decoded encoded information about an echo chat message.
---@param info MessageInfo
---@param player IsoPlayer
---@private
function Transformation._decodeEcho(info, player)
...
---Decodes roleplay language information.
---@param info MessageInfo
---@param player IsoPlayer
---@private
function Transformation._decodeLanguages(info, player)
...
---Decodes mentions encoded in a message.
---@param info MessageInfo
---@private
function Transformation._decodeMentions(info)
    local text = info.content or info.rawText

    local mentionPattern = API._metadataFormatters.mention:getPattern(false, true)
    info.content = text:gsub(mentionPattern, Transformation._decodeSingleMention)
end

---Decodes information about a narrative style message.
---@param info MessageInfo
---@private
function Transformation._decodeNarrativeStyle(info)
...
---Replacement function for an encoded mention.
---@param match string
---@return string
---@private
function Transformation._decodeSingleMention(match)
...
---Decodes the message stream.
---@param info MessageInfo
---@private
function Transformation._decodeStream(info)
    if info.chatType == 'radio' then
        return
    end
...
---@param info MessageInfo
---@private
function Transformation._formatMentions(info)
...
---Sets up tokens and tags based on the chat type.
---@param info MessageInfo
---@param player IsoPlayer
---@private
function Transformation._setup(info, player)
